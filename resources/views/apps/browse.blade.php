<x-guest-layout>
    ...
    <section>
        ...
        <div class="flex flex-col justify-end mt-4 bg-black/10 bg-blend-multiply rounded-3xl h-80 overflow-hidden bg-cover bg-center px-7 pt-4 pb-6 text-white"
         style="background-image: url('storage/image/{{$cover->cover}}');" >
            ...
            <div class="bg-gradient-to-r from-black/30 to-transparent -mx-7 -mb-6 px-7 pb-6 pt-2">
                <span class="uppercase text-3xl font-semibold drop-shadow-lg ">{{$cover->title}}</span>
                ...
                <div class="mt-4 flex space-x-3 items-center">
                    <a href="{{route('moviesdetail', $cover->id)}}" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 rounded-lg text-xs inline-block">Watch</a>
    ...
    <x-actor-section :actors="$actors" />

    <x-movies-section :movies="$movies"/>
</x-guest-layout>